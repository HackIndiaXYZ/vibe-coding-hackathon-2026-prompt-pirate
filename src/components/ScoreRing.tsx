export function ScoreRing({
  ...
  const gradId = `ring-grad-${label ?? "x"}`;
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <div className="font-display text-5xl leading-none tracking-tight">
          {value}
          <span className="text-base text-muted-foreground font-sans align-top ml-0.5">/100</span>
        </div>
        {label && <div className="mt-1 text-[10px] font-mono uppercase tracking-[0.22em] text-muted-foreground">{label}</div>}
